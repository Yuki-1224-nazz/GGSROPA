...
// Only serve static files in non-Vercel environments (local development)
...
async function getBrowser() {
  if (IS_VERCEL) {
    // Use @sparticuz/chromium with playwright-core for Vercel
...
app.get('/', (req, res) => {
  if (IS_VERCEL) {
    return res.json({ 
      status: 'ok', 
      message: 'Turnstile Solver API is running',
      endpoints: {
        health: '/api/health',
        stats: '/api/stats',
...
// ─────────────────────────────────────────────────────────────────────────────
// Local server (only run when this file is executed directly, not imported)
// ─────────────────────────────────────────────────────────────────────────────
if (!IS_VERCEL && require.main === module) {
